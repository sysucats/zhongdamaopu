module.exports = async (ctx) => {
    ...
    async function deleteFiles(photoUrls) {
        ...
        const { OSS_ENDPOINT, OSS_BUCKET, OSS_SECRET_ID, OSS_SECRET_KEY } = app_secret;
        ...
        try {
            for (const url of photoUrls) {
                ...
                const result = { url, fileName, success: false, error: null };
                try {
                    // 删除文件
                    await new Promise((resolve, reject) => {
                        cos.deleteObject(
                            ...
                            (err, data) => {
                                console.log(`Delete result for ${fileName}:`, { err, data });
                                if (err) {
