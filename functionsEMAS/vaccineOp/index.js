module.exports = async (ctx) => {
    ...
    const { result: is_manager } = await ctx.mpserverless.function.invoke('isManager', { openid: openid, req: 2 })
    if (!is_manager) {
    ...
    if (operation === 'add') {
        ...
        try {
            const { result } = await ctx.mpserverless.db.collection('vaccine').insertOne(vaccineData);
        ...
        try {
            const { result: vaccine } = await ctx.mpserverless.db.collection('vaccine').findOne({ _id: vaccine_id });
            return { msg: '获取成功', result: true, data: vaccine };
        } catch (error) {
        ...
        try {
            const { result: vaccines } = await ctx.mpserverless.db.collection('vaccine').find({ cat_id: cat_id }, { sort: { vaccine_date: -1 } });
            return { msg: '获取成功', result: true, data: vaccines };
        } catch (error) {
            return { msg: '获取失败', error, result: false };
        }
        ...
        try {
            const { result } = await ctx.mpserverless.db.collection('vaccine').updateOne({ _id: vaccine_id }, { $set: vaccineData });
        ...
        try {
            const { result } = await ctx.mpserverless.db.collection('vaccine').deleteOne({ _id: vaccine_id });
        ...
        // 获取所有类型
        try {
            const { result: types } = await ctx.mpserverless.db.collection('setting').findOne({ _id: 'vaccine_type' });
            return { msg: '获取成功', result: true, data: types };
        } catch (error) {
            return { msg: '获取失败', error, result: false };
        }
        ...
        try {
            ...
            await ctx.mpserverless.db.collection('setting').updateOne({ _id: 'vaccine_type' }, { $set: { types: typeList } });
        ...
        try {
            ...
            // 检查是否在使用中
            const { result: inUseCount } = await ctx.mpserverless.db.collection('vaccine').count({ vaccine_type: typeToRemove });
            ...
            await ctx.mpserverless.db.collection('setting').updateOne({ _id: 'vaccine_type' }, { $set: { types: newTypeList } });
        ...
        try {
            // 获取所有符合条件的疫苗记录
            const { result: vaccines } = await ctx.mpserverless.db.collection('vaccine').find({ vaccine_type: vaccine_type }, { sort: { vaccine_date: -1 } });
            ...
            // 获取所有猫咪信息
            const { result: cats } = await ctx.mpserverless.db.collection('cat').find({ _id: { $in: catIds } }, { projection: { name: 1, campus: 1, area: 1, avatar: 1, photo_count_best: 1 } });
