...
export default async function (ctx: FunctionContext) {
  ...
  try {
    ...
    const query = _.or([
      { adopt: _.exists(false) },
      { missing: _.exists(false) },
      { gender: _.exists(false) },
      { to_star: _.exists(false) },
      { sterilized: _.exists(false) }
    ]);

    // 获取符合条件的猫记录
    const result = await db.collection('cat').where(query).limit(limit).get();
    ...
    for (const cat of cats) {
      ...
      // 更新记录
      const updateResult = await db.collection('cat').doc(cat._id).update(updateData);
