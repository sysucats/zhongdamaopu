module.exports = async (ctx) => {
    ...
    if (badgeCode != undefined) {
        const { result: codeItem } = await ctx.mpserverless.db.collection('badge_code').findOne({ code: badgeCode });
        if (!codeItem) {
    ...
    // 获取徽章配置
    const { result: badgeDefCount } = await ctx.mpserverless.db.collection('badge_def').count({});
    let badgeDef = [];
    while (badgeDef.length < badgeDefCount) {
        const { result: tmp } = await ctx.mpserverless.db.collection('badge_def').find({}, { skip: badgeDef.length });
    ...
    // 修改兑换码
    if (badgeCode != undefined) {
        await ctx.mpserverless.db.collection('badge_code').updateOne({ code: badgeCode }, { $set: { useTime: new Date(), useOpenid: openid } });
    }
