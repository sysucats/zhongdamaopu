module.exports = async (ctx) => {
    const { result: is_manager } = await ctx.mpserverless.function.invoke('isManager', { openid: ctx.args.openid, req: 2 })
    if (!is_manager) {
    ...
    if (cat_id) {
        ...
        return await ctx.mpserverless.db.collection('cat').updateOne({ _id: cat_id }, { $set: cat_data });
    } else {
        const { result: count } = await ctx.mpserverless.db.collection('cat').count({});
        var _no = 'c' + count;
        while (true) {
            var { result: existNum } = await ctx.mpserverless.db.collection('cat').count({ _no: _no });
            if (!(existNum > 0)) {
                break;
            }
            // 加上俩随机字符
